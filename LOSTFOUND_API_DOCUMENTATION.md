...
- **Headers:** 
  - `Authorization: Bearer {token}`
  - `Content-Type: application/json`
- **Description:** Confirm item handover using QR code token. Allows receiver to confirm receiving the item from giver without needing to be the item owner.
- **Body:**
```json
{
...
```
- **After Handover:**
  - `returnedUserId` is set to the receiver (person who scanned QR and confirmed)
  - `status` changes to `returned`
  - History record is created with giver (foundUserId) and receiver (returnedUserId)
...
```
- **Notes:**
  - **No ownership check** - Any authenticated user can confirm handover with valid QR token
  - Automatically changes item status from `found` → `returned`
  - Sets `returnedUserId` to current user (receiver)
